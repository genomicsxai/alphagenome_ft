...
def forward_with_encoder_output(
    ...
    Module calls go through :func:`alphagenome_ft.compat.call_with_is_training`, so
    this works with ``alphagenome_research`` builds from before and after the June 2026
    change that made ``is_training`` a required keyword-only argument. The trunk is run
    with ``is_training=False`` (inference-mode batch-norm statistics), matching
    ``AlphaGenome.__call__`` and the previous ``alphagenome_ft`` behaviour.
    ...
    # ``skip_x`` is keyword-only in newer alphagenome_research; keywords work on both APIs.
